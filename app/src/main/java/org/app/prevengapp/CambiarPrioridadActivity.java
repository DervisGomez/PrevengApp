...
public class CambiarPrioridadActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cambiar_prioridad);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        tvNumero=(TextView)findViewById(R.id.tvNumeroCamPri);
        tvTitulo=(TextView)findViewById(R.id.tvTituloCamPri);
        tvPrioridad=(TextView)findViewById(R.id.tvPrioridadCamPri);
        spPrioridad=(Spinner)findViewById(R.id.spPrioridadCamPri);
        btnGuardar=(Button)findViewById(R.id.btnCambiarPrioridad);
    ...
    public void onClick(View view) {
        switch (view.getId()){
            case R.id.btnCambiarPrioridad:
                if (encontrado){
                    int tip=spPrioridad.getSelectedItemPosition();
                    if (tip<0||tip==lista.length-1){
                        showAlertDialog(CambiarPrioridadActivity.this,"Cambiar","Seleccione un tipo de usuario",false);
                    }else{
                        pd = ProgressDialog.show(this, "Cambiar", "Cargando...", true, false);
                        control=2;
                        Bundle bolsa=getIntent().getExtras();
                ...
                if(!array.isJsonNull()) {
                    ...
                    for (int x = 0; x < array.size(); x++) {
                        ...
                        if (!(objO.get("PRIORIDAD").isJsonNull())) {
                            lista[x]=objO.get("PRIORIDAD").getAsString();
                        }
                    }
                    lista[array.size()]="Seleccione una prioridad";
                    ...
                    for (int x = 0; x < array.size(); x++) {
                        ...
                        if (!(objO.get("REPOCONS").isJsonNull())) {
                            ...
                            if (!objO.get("REPOCONS").getAsString().equals("0")){
                                showAlertDialog2(CambiarPrioridadActivity.this,"Cambiar","Cambio realizado exitosamente",true);
